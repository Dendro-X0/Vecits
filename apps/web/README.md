...
- provides a marketplace draft builder for `ServiceOffer`, `ServiceOrder`, `SpendCredits` (escrow), `ServiceDelivery`, `ServiceAccept`, `ServiceDispute`, `ServiceSettle`, `ServiceCancel`, and `OrderAmend` (draft -> sign -> submit; cancel/amend have no dedicated wizard UI yet)
